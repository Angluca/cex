...
_os__fs__find_walker(const char* path, os_fs_stat_s ftype, void* user_ctx)
{
    ...
    // allocate new string because path is stack allocated buffer in
    // os__fs__dir_walk()
...
static arr$(char*) cex_os__fs__find(const char* path, bool is_recursive, IAllocator allc)
{
    ...
    if (!is_recursive) {
        ...
        if (f.is_valid && f.is_file) {
            // Find used with exact file path, we still have to return array +
            // allocated path copy
            arr$(char*) result = arr$new(result, allc);
...
cex_os__cmd__create(os_cmd_c* self, arr$(char*) args, arr$(char*) env, os_cmd_flags_s* flags)
{
    ...
    for (u32 i = 0; i < args_len - 1; i++) {
        if (args[i] == NULL) {
            return "one of `args` items is NULL, which may indicate string operation "
                   "failure";
        }
    }
...
cex_os__cmd__run(const char** args, usize args_len, os_cmd_c* out_cmd)
{
    ...
    for (u32 i = 0; i < args_len - 1; i++) {
        if (args[i] == NULL || args[i][0] == '\0') {
            return e$raise(
                Error.argument,
                "`args` item[%d] is NULL/empty, which may indicate string "
                "operation failure",
                i
            );
        }
...
static Exception
cexy__cmd__check(int argc, char** argv, void* user_ctx)
{
    (void)argc;
    (void)argv;
    (void)user_ctx;
