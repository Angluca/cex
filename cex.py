...
class CEXCompiler:
    ...
    def compile_app(
        ...
        src_incl_path = os.path.join(os.getcwd())
...
class CEXBuilder:
    ...
    def action_create(self, proj_name):
        assert os.path.exists(self.project_root)
        assert re.match(r"\w+", proj_name), f"{proj_name=} must be alphanumeric"

        src_path = os.path.join(self.project_root, "src")
        os.makedirs(src_path, exist_ok=True)

        app_dir = os.path.join(src_path, proj_name)
        assert not os.path.exists(app_dir), f"Already exists: {app_dir}"
        os.makedirs(app_dir)

        template_dir = os.path.join(self.env.root, "examples", "project_template")

        with open(os.path.join(template_dir, "src", "App.h"), "r") as fh:
            contents = fh.read()
            contents = contents.replace("App", proj_name)
            with open(os.path.join(app_dir, f"{proj_name}.h"), "w") as fh_out:
                fh_out.write(contents)

        with open(os.path.join(template_dir, "src", "App.c"), "r") as fh:
            contents = fh.read()
            contents = contents.replace("App", proj_name)
            with open(os.path.join(app_dir, f"{proj_name}.c"), "w") as fh_out:
                fh_out.write(contents)

        with open(os.path.join(template_dir, "src", "main.c"), "r") as fh:
            contents = fh.read()
            contents = contents.replace("App", proj_name)
            with open(os.path.join(app_dir, "main.c"), "w") as fh_out:
                fh_out.write(contents)

        test_dir = os.path.join(self.project_root, "tests")
        if os.path.exists(test_dir):
            test_dir_app = os.path.join(test_dir, f"app_{proj_name}")
            assert not os.path.exists(
                test_dir_app
            ), f"test dir already exists: {test_dir_app}"

            os.makedirs(test_dir_app)

            with open(os.path.join(template_dir, "tests", "test_App.c"), "r") as fh:
                contents = fh.read()
                contents = contents.replace("App", proj_name)
                contents = contents.replace(
                    f"src/{proj_name}", f"src/{proj_name}/{proj_name}"
                )
                test_fn = os.path.join(test_dir_app, f"test_{proj_name}.c")
                with open(test_fn, "w") as fh_out:
                    fh_out.write(contents)

    def action_clean(self):
        build_path = os.path.join(self.project_root, "build")
        if os.path.exists(build_path):
            print(f"removing: {build_path}")
            shutil.rmtree(build_path)

        test_build_path = os.path.join(self.project_root, "tests", "build")
        if os.path.exists(test_build_path):
            print(f"removing: {test_build_path}")
            shutil.rmtree(test_build_path)

    def build(self):
        if self.action == "create":
            self.action_create(self.app_name)
        elif self.action == "build":
            self.action_build(
                target=self.target,
...
class CEXProcessor:
        ...
        def process_line(self, line: str) -> bool | None:
            ...
            while i < len(line):
                ...
                if self._is_signature:
                    ...
                    # Example:
                    #  struct {
                    #    int field;   // this is my comment it's  breaking because of ' in it's
                    if not self._in_string_quotes and c != '"' and c != "'":
                        ...
                        if not self._in_string_quotes:
                            ...
                            if c == '"' or c == "'":
                                ...
                                # NOTE: special case "'" or '"'
                                if i >= 2 and (
                                    line[i - 1] != "\\"
                                    or (line[i - 1] == "\\" and line[i - 2] == "\\")
                                    or self._in_string_quotes[0] == c
                                ):
                                    if self._in_string_quotes[0] == c:
                                        # escaped quote, skipping \" or " '  "
    ...
    class Func:
        ...
        def __init__(self, decl: str, ifc_prefix: str):
            ...
            assert ifc_prefix, "empty ifc_prefix"
        ...
        def is_public_cex(self) -> bool:
            ...
            g2 = re.match(rf"^{self.ifc_prefix}_([a-z]\w+)", self.name, re.MULTILINE)
        ...
        def to_pointer_decl(self, indent: bool) -> str:
            ...
            if indent:
                ...
                return f"{self.typedef:<15} (*{self.ifc_name})({self.args});"
    ...
    Function:
        ...
        is_public_cex: {self.is_public_cex}
        ifc_prefix: {self.ifc_prefix}
        name: {self.name}
        args: {self.args}
        """
    ...
    def make_interface_declaration(self, blocks: list[Block]):
        ...
        for sub_module, flist in functions.items():
            ...
            for func in sorted(flist, key=lambda x: x.name):
                if func.docstr:
                    ifc_contents.append(dd(func.docstr))
                ifc_contents.append(dd(func.to_pointer_decl(indent=False)))
    ...
    def make_fff_fake(
        ...
        base_file = os.path.join(fake_dir, file)
        if sub_module:
        ...
        src.write("// NOTE: include in test as\n")
        src.write(f'//#include "{base_file}"\n\n')
        src.write("\n// clang-format off\n")
        src.write("#include <cex/test/fff.h>\n")
        src.write("#include <cex/cex.h>\n")
        src.write(f'#include "{file}"\n\n')
        ...
        # fmt: off
        src_reset.write(
            f"\n\nstatic void fake__{os.path.basename(base_file).replace('.h', '__resetall(void) {')}\n"  # noqa
        )
        src_wrap_reset.write(
            f"\n\nstatic void fake__{os.path.basename(wrap_fake).replace('.h', '__resetall(void) {')}\n"  # noqa
        )
        # fmt: on
        ...
        # print(src.getvalue())
        with open(os.path.join(base_file), "w") as fh:
            fh.write(src.getvalue())
            fh.write(src_reset.getvalue())

        # TODO: decide if __wrap fake is viable option
        # if not sub_module:
...
def action_proj(args):
    proj_name = args.proj_name
    assert not os.path.exists(proj_name), f"project path already exist: {proj_name}"

    print(f"making new CEX project: {proj_name}")
    env = CEXEnv()
    shutil.copytree(os.path.join(env.root, "examples", "project_template"), proj_name)

    sbuf = io.StringIO()
    with open(os.path.join(proj_name, "Makefile"), "r") as fh:
        sbuf.write(fh.read())

    # Adding `make run` for current project app
    sbuf.write("run:\n")
    sbuf.write(f"\t./build/{proj_name} --name=new_cex Hello world, place your args!\n")

    with open(os.path.join(proj_name, "Makefile"), "w") as fh:
    ...
    if os.path.isdir(src_path):
        ...
        for fn in glob.glob(f"{src_path}/**/*.c", recursive=True):
            ...
            if fn.endswith("include/cex.c"):
                ...
                continue
            sources[fn] = None
        for fn in glob.glob(f"{src_path}/**/*.h", recursive=True):
            if os.path.basename(fn).startswith("test_") or os.path.basename(
                fn
            ).startswith("fake_"):
                continue
...
if __name__ == "__main__":
    ...
    process_parser.add_argument(
        "source_path",
        help=(  # noqa
            "Directory (batch update mode), or file path .c (code mode) or .h (lib"
            " mode)"
        ),
    )
    # process_parser.add_argument("--fakedir", help="fff.h fake directory", default="tests/fake/")
    process_parser.add_argument("--fakedir", help="fff.h fake directory", default=None)
    process_parser.add_argument(
        "--namespace",
    ...
    build_parser.add_argument(
        "action",
        help="app action {create, build, clean}",
    )
    build_parser.add_argument(
        "app_name",
        help=(  # noqa
            ". for src/main.c, or `my_app_name` for src/<my_app_name>/main.c, or `all`"
        ),
    )
    build_parser.add_argument(
        "-o",
    ...
    build_parser.add_argument(
        "--target",
        help=(  # noqa
            "compiler args preset: {debug, release-safe, release-fast, custom},"
            " default: debug"
        ),
        default="debug",
    )
    ...
    build_parser.set_defaults(func=action_app)

    proj_parser = subparsers.add_parser("project", help="Creates new CEX project")
    proj_parser.add_argument(
        "proj_name",
        help="Project directory name",
    )
    proj_parser.set_defaults(func=action_proj)
